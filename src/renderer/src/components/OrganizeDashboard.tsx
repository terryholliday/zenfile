    ...
    >
      ...
      <div className="mt-auto">
        <label className="flex items-center gap-2 mb-3 cursor-pointer group/chk">
          <div
            className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${generateDna ? 'bg-indigo-500 border-indigo-500' : 'border-neutral-600 bg-black/40 group-hover/chk:border-neutral-500'}`}
          >
            {generateDna && (
              <svg
          ...
          />
          <span
            className={`text-xs font-medium transition-colors ${generateDna ? 'text-indigo-300' : 'text-neutral-500'}`}
          >
    ...
    <div className="h-full p-6 overflow-hidden flex flex-col">
      <header className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-neutral-400 max-w-xl">
            AI-powered organization recommendations. Group loose files into logical folders with one
            click.
          </p>
        </div>
        {/* Undo Button */}
      ...
      {isAnalyzing ? (
        <div className="flex-1 flex flex-col items-center justify-center">
          ...
          <div className="text-sm text-indigo-300 font-bold tracking-widest uppercase">
            Analyzing Context...
          </div>
        </div>
      ) : stacks.length > 0 ? (
