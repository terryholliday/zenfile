...
function StackCard({ stack, onExecute, onDismiss }: { stack: SmartStack; onExecute: (id: string) => void; onDismiss: (id: string) => void }) {
  return (
    <motion.div
      layout
    ...
    >
      <div className="flex justify-between items-start">
        ...
        <div className="text-xs font-mono text-neutral-400 bg-black/20 px-2 py-1 rounded">{stack.files.length} files</div>
      </div>
      ...
      <div className="flex-1 min-h-[80px] bg-black/20 rounded-lg p-2 flex flex-col gap-1 overflow-hidden">
        ...
        {stack.files.length > 3 && <div className="text-[10px] text-neutral-500 italic pl-3">+ {stack.files.length - 3} more</div>}
      </div>

      <div className="flex gap-2 mt-auto pt-2">
        <button onClick={() => onDismiss(stack.id)} className="flex-1 py-2 rounded-lg text-xs font-bold text-neutral-400 hover:bg-white/5 transition-colors">
          Ignore
        </button>
        <button
  ...
  return (
    <div className="h-full p-6 overflow-hidden flex flex-col">
      <header className="mb-6">
        ...
        <p className="text-neutral-400 max-w-xl">AI-powered organization recommendations. Group loose files into logical folders with one click.</p>
      </header>
      ...
      {isAnalyzing ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 overflow-y-auto pb-10 scrollbar-hide">
          <AnimatePresence>
            {stacks.map((stack) => (
              <StackCard key={stack.id} stack={stack} onExecute={executeStack} onDismiss={dismissStack} />
            ))}
          </AnimatePresence>
        </div>
      ) : (
